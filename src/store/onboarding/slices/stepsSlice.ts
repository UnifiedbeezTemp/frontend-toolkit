...
const stepsSlice = createSlice({
  ...
  reducers: {
    ...
    setSteps: (
      ...
      if (state.visitedSteps.length === 0) {
        ...
        const existsIndex = state.visitedSteps.findIndex(
          (v) =>
            v.stepId === initialVisitedStep.stepId &&
            v.subStepId === initialVisitedStep.subStepId,
        );
        if (existsIndex === -1) {
    ...
    navigateToStep: (
      ...
      const targetStepIndex = state.steps.findIndex(
        (step) => step.id === stepId,
      );
      const targetSubStepIndex = targetStep.subSteps.findIndex(
        (sub) => sub.id === subStepId,
      );
      ...
      state.steps = state.steps.map((step, sIdx) => {
        ...
        return {
          ...
          subSteps: step.subSteps.map((subStep, ssIdx) => {
            const isPastSubStep =
              isPastStep || (isCurrentStep && ssIdx < targetSubStepIndex);
            const isTargetSubStep =
              isCurrentStep && ssIdx === targetSubStepIndex;
    ...
    completeCurrentSubStep: (state) => {
      ...
      const updatedStep = state.steps.find(
        (step) => step.id === state.currentStepId,
      );
      if (updatedStep) {
