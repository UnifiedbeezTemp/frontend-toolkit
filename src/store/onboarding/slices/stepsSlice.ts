  ...
  reducers: {
    ...
    setSteps: (state, action: PayloadAction<Step[]>) => {
      ...
      if (action.payload.length > 0 && action.payload[0].subSteps.length > 0) {
        ...
        state.visitedSteps = [{ stepId: firstStep.id, subStepId: firstSubStep.id }];
      }
    },

    // Navigate to a specific step and sub-step
    navigateToStep: (state, action: PayloadAction<{ stepId: number; subStepId: string | number }>) => {
      ...
      const visitedIndex = state.visitedSteps.findIndex(
        visited => visited.stepId === stepId && visited.subStepId === subStepId
      );
    ...
    completeCurrentSubStep: (state) => {
      ...
      const allSubStepsCompleted = currentStep.subSteps.every(sub => sub.isCompleted);
    ...
    updateSubStepFormData: (state, action: PayloadAction<{ formData: any }>) => {
      state.steps = state.steps.map(step =>
        step.id === state.currentStepId
          ? {
              ...step,
    ...
    jumpToLatestStep: (state) => {
      if (state.visitedSteps.length > 0) {
        ...
        stepsSlice.caseReducers.navigateToStep(state, {
          payload: { stepId: latestVisited.stepId, subStepId: latestVisited.subStepId },
          type: 'navigateToStep',
        });
      }
    },
