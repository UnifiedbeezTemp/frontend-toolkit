...
import {
  useDelayBeforeFollowUpField,
  OPTIONS,
} from "./hooks/useDelayBeforeFollowUpField";

interface DelayBeforeFollowUpFieldProps {
  value: string | null;
  onChange: (value: string) => void;
}

export default function DelayBeforeFollowUpField({
  value,
  onChange,
}: DelayBeforeFollowUpFieldProps) {
  const icons = useSupabaseIcons();
  const { isOpen, triggerRef, handleToggle, handleClose, handleSelect } =
    useDelayBeforeFollowUpField(value, onChange);
