...
export default function FileItem({ file, onRemove, onCancel, isEditing = false}: FileItemProps) {
  ...
  return (
    <div className="border-border border py-[1.6rem] px-[0.8rem] rounded-[0.8rem]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-[1rem]">
          <div className="relative shrink-0 flex-3">
            <ImageComponent
              ...
              alt=""
            />
            <div className={`absolute ${getFileTypeColor(file.type)} text-white w-full flex items-center justify-center text-center text-[.8rem] px-[2px] rounded-[1px] top-[50%]`}>
              {file.type}
            </div>
          </div>
          <Text>{file.name}</Text>
        ...
        {!isEditing && <button 
          onClick={() => file.status === 'uploading' ? onCancel(file.id) : onRemove(file.id)}
